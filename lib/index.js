...
class MohError extends Error {
  constructor(...args) {
    ...
    let err, message, info, status, isInvalidArgs
    ...
    switch (args.length) {
      case 1:
        if (!isString(args[0]) && !isObject(args[0]) && !isError(args[0])) {
          isInvalidArgs = true
          break
        }
        if (isError(args[0])) err = args[0]
        if (isObject(args[0])) info = args[0]
        if (isString(args[0])) message = args[0]
        break
      case 2:
        if (
          (!isString(args[0]) && !isError(args[0])) ||
    ...
    if (isInvalidArgs) {
      debug(
        'Failed to generate MohError, please check args: ' +
          '\n with 1 arg: String || Error ||Object ' +
          '\n with 2 args: [String, Error], [Object, StatusCode(/^[1-5][0-9][0-9]$/)]'
      )
